...
const ROLES = {
  student: {
    name: 'Student',
    icon: GraduationCap,
    description: 'Access grades, schedules, assignments, and school announcements.',
    color: 'bg-sky-600 hover:bg-sky-700', // Updated from blue
    hoverRing: 'ring-sky-500/50', // Updated from blue
    route: '/student',
  },
  parent: {
    name: 'Parent',
    icon: Users,
    description: 'Monitor your child’s progress, attendance, and communicate with staff.',
    color: 'bg-emerald-600 hover:bg-emerald-700', // Updated from green
    hoverRing: 'ring-emerald-500/50', // Updated from green
    route: '/parent',
  },
  teacher: {
    name: 'Teacher',
    ...
    color: 'bg-violet-600 hover:bg-violet-700', // Updated from indigo
    hoverRing: 'ring-violet-500/50', // Updated from indigo
    route: '/teacher',
  },
};
 ...
 */
const RoleCard = ({ roleKey }) => {
  const role = ROLES[roleKey];
  const Icon = role.icon;

  // Determine the specific color classes based on the new scheme
  const borderColorClass = roleKey === 'student' 
    ? 'border-sky-500' 
    : roleKey === 'parent' 
      ? 'border-emerald-500' 
      : 'border-violet-500';

  const iconColorClass = roleKey === 'student' 
    ? 'text-sky-500' 
    : roleKey === 'parent' 
      ? 'text-emerald-500' 
      : 'text-violet-500';
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-4 text-xl font-bold text-gray-800">{role.name} Login</h3>
      <p className="mt-2 text-center text-gray-600 text-sm">{role.description}</p>
      <button
        className={`
        ...
        <div className="bg-sky-50 py-16 md:py-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-gray-900">
              Your Complete <span className="text-sky-600">School Management</span> Solution
            </h1>
            <p className="mt-4 max-w-2xl mx-auto text-xl text-gray-500">
              Connecting students, parents, and educators in one seamless digital ecosystem for better learning outcomes.
            </p>
            <div className="mt-8">
              <span className="text-lg font-semibold text-gray-700">Select your role to continue:</span>
            </div>
          </div>
        </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                    <div className="p-6 bg-gray-100 rounded-xl shadow-lg">
                        ...
                        <h3 className="text-xl font-semibold text-gray-800">Unified Access</h3>
                        <p className="mt-2 text-gray-600">One place for all stakeholders—students, parents, and teachers—to find relevant information.</p>
                    </div>
                    <div className="p-6 bg-gray-100 rounded-xl shadow-lg">
                        ...
                        <p className="mt-2 text-gray-600">Instant updates on grades, attendance, and assignment status for timely intervention.</p>
                    </div>
                    <div className="p-6 bg-gray-100 rounded-xl shadow-lg">
                        ...
                        <p className="mt-2 text-gray-600">Enterprise-grade security to protect sensitive student and academic records.</p>
                    </div>
                </div>
            </div>
        </section>
